...
internal static class Strings
{
    ...
    [
        ...
        ("Toolbar.Enable",             "Enable Openness", "启用真实后端"),
        ("Toolbar.Enable.Tip",         "Build the Openness adapter against the TIA Portal on this machine. Needed once, because Siemens does not allow shipping it prebuilt.",
                                       "针对本机的 TIA Portal 编译 Openness 适配器。只需一次——西门子不允许随包分发预编译版本。"),
        ("Status.BuildingAdapter",     "Building the Openness adapter",
                                       "正在编译 Openness 适配器"),
        ("Status.AdapterBuilt",        "Openness adapter built against V{0}. Connect to use it.",
                                       "已针对 V{0} 编译出 Openness 适配器,现在可以连接。"),
        ("Status.AdapterFailed",       "The adapter did not compile against V{0} - {1} error(s). See the log.",
                                       "适配器未能针对 V{0} 编译通过,共 {1} 处错误,详见日志。"),
        ("Log.AdapterBuilt",           "Openness adapter built against V{0} using {1} Siemens assembly/assemblies -> {2}",
                                       "已针对 V{0} 使用 {1} 个西门子程序集编译出 Openness 适配器 -> {2}"),
        ("Log.AdapterHint",            "Each error names a file and line in the adapter sources. A missing Siemens type or member means this build expects an API your TIA version does not have; please report it.",
                                       "每条错误都指向适配器源码的文件和行号。若提示 Siemens 类型或成员缺失,说明本版本期望的 API 在你的 TIA 版本中不存在,请反馈。"),
